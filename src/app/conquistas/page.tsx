...
export default function AchievementsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="mb-8">
        <h1 className="font-[var(--font-display)] text-2xl font-semibold tracking-tight">{t("achievements.title")}</h1>
        <p className="mt-1 text-sm text-[var(--color-text-muted)]">
          {t("achievements.unlockedCount", { unlocked: unlockedCount, total: list.length })}
        </p>
      </div>
              ...
              >
                <div
                  className={cn(
                    ...
                    a.unlocked ? "bg-gradient-to-br from-[var(--color-gold)]/25 to-[var(--color-teal)]/25" : "bg-[var(--color-overlay)]"
                  )}
                >
                  {a.unlocked ? (
                    <DynamicIcon name={a.icon} className="h-6 w-6 text-[var(--color-gold-soft)]" />
                  ) : (
                    <Lock className="h-5 w-5 text-[var(--color-text-faint)]" />
                  )}
                </div>
                <div>
                  <p className="font-[var(--font-display)] text-sm font-semibold">{title}</p>
